...
* BUG-002: 検索結果が0件になる (open)
* BUG-005: 商品画像が表示されない (open)
* BUG-006: カートに追加後に金額がずれる (in_progress)
* BUG-013: 決済完了メールが二重送信される (open)
* BUG-018: Androidアプリのメモリリーク (in_progress)
* BUG-025: 商品レビューが投稿できない (open)
* BUG-026: CI/CDパイプラインが失敗する (open)
* BUG-028: APIのレートリミットが機能しない (open)
* BUG-030: ユーザー登録時にメールアドレスの重複チェックが失敗する (open)
* BUG-032: データウェアハウスのクエリが5分以上かかる (in_progress)
* BUG-033: 推薦アルゴリズムが同じ商品を繰り返す (open)
* BUG-036: アクセスログの集計が日付をまたぐと失敗する (open)
* BUG-037: APIゲートウェイの認証トークン検証が遅い (open)
* BUG-042: XSS脆弱性がコメント投稿フォームに存在 (open)
* BUG-044: セッションフィクセーション攻撃が可能 (open)
* BUG-052: 決済APIのタイムアウト設定が短すぎる (open)
* BUG-055: 管理者のパスワード有効期限が設定されていない (open)
* BUG-056: データウェアハウスへのETLが冪等でない (open)
* BUG-060: APIゲートウェイのキャッシュが正しくパージされない (open)
* BUG-061: Frontend画面の初期ロードが5秒以上かかる (open)
* BUG-064: 商品の在庫数がリアルタイムに反映されない (open)
* BUG-069: 月次レポートの合計値が合わない (open)
* BUG-070: データパイプラインの監視アラートが機能しない (open)
* BUG-073: 脆弱性スキャナがCVEデータベースを更新しない (open)
* BUG-075: APIゲートウェイでリクエストが稀にドロップする (open)
* BUG-077: DataEngineeringチームのジョブスケジューラが重複起動する (open)
* BUG-078: 機械学習のバッチ推論が週末にスキップされる (open)
* BUG-080: プラットフォームチームのKubernetesポッドが頻繁に再起動する (in_progress)
* BUG-081: Backendのメモリ使用量が時間で増加する (open)
* BUG-088: MLモデルの予測に偏り（バイアス）が検出された (open)
* BUG-090: プラットフォームのオートスケールが機能しない (open)
* BUG-092: 推薦APIのレスポンスタイムが2秒を超える (open)
* BUG-093: 脆弱性スキャンレポートが暗号化されていない (open)
* BUG-098: 管理者画面の操作ログが記録されていない (open)
