...
以下の高優先度（severity: high）のバグ一覧を教えてください。

- BUG-002: 検索結果が0件になる
- BUG-005: 商品画像が表示されない
- BUG-006: カートに追加後に金額がずれる
...
- BUG-032: データウェアハウスのクエリが5分以上かかる
- BUG-033: 推奨アルゴリズムが同じ商品を繰り返す
...
- BUG-092: 推奨APIのレスポンスタイムが2秒を超える
- BUG-093: 脆弱性スキャンレポートが暗号化されていない
- BUG-098: 管理者画面の操作ログが記録されていない

これらのバグは全て高優先度で、速やかに対応することが求められています。
...
解決済み（resolved）のバグのタイトルは、以下の通りです。
